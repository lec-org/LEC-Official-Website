...
/** 编辑表单显示和提交成员所需的初始数据。 */
export type MemberFormInitialValues = { id: string; grade: string; name: string; dest: string; type: string; qq?: string };

/** 新建与编辑成员共用表单的输入参数。 */
type MemberFormProps = { mode: "create" | "edit"; grades?: string[]; initialValues?: MemberFormInitialValues };
...
/** 管理员用于新增或编辑往届成员的表单。 */
export function MemberForm({ mode, grades = [], initialValues }: MemberFormProps) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage("");
    const response = await fetch(mode === "create" ? "/api/members" : `/api/members/${initialValues?.id}`, {
  ...
  return (
    <form className="space-y-4" onSubmit={submit}>
      ...
      <datalist id="member-grade-options">
